...
void LCD_Gpio_Init(void) {
    lcd_spi2.Instance = SPI2;
    lcd_spi2.Init.Mode = SPI_MODE_MASTER;
    lcd_spi2.Init.Direction = SPI_DIRECTION_2LINES;
    lcd_spi2.Init.DataSize = SPI_DATASIZE_8BIT;
    lcd_spi2.Init.CLKPolarity = SPI_POLARITY_HIGH;
    lcd_spi2.Init.CLKPhase = SPI_PHASE_2EDGE;
    lcd_spi2.Init.NSS = SPI_NSS_SOFT;
    lcd_spi2.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_2;
    lcd_spi2.Init.FirstBit = SPI_FIRSTBIT_MSB;
    lcd_spi2.Init.TIMode = SPI_TIMODE_DISABLE;
    lcd_spi2.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
    lcd_spi2.Init.CRCPolynomial = 7;
    lcd_spi2.Init.CRCLength = SPI_CRC_LENGTH_DATASIZE;
    lcd_spi2.Init.NSSPMode = SPI_NSS_PULSE_DISABLE;
    if (HAL_SPI_Init(&lcd_spi2) != HAL_OK) {
        Error_Handler();
    }
}
...
uint8_t SPI2_WriteByte(uint8_t *TxData, uint16_t size) {
    return HAL_SPI_Transmit(&lcd_spi2, TxData, size, 1000);
}
