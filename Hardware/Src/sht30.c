 ...
 * @retval    成功 —— 返回HAL_OK
*/
static uint8_t SHT30_Read_Dat(uint8_t *dat) {
...
*/
static uint8_t SHT30_Dat_To_Float(uint8_t *const dat, float *temperature, float *humidity) {
 ...
 * @retval 成功返回HAL_OK
 */
uint8_t SHT30_Get_Temp_Hum(float *temperature, float *humidity) {
    uint8_t dat[6];
    SHT30_Read_Dat(dat);
